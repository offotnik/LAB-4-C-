...
Для решения данной задачи, напишем метод DeleteFirst, который принимает LinkedList. С помощью метода Contains() находим слово, которое надо удалить, и методом Remove() удаляем его.
...
Для решения данной задачи, напишем методы GetTracksLikedByAll, GetTracksLikedBySome, GetTracksLikedByNone, AnalyzePreferences, PrintResult. Мы совершаем операции на множества и выводим: что нравится всем, что нравится некоторым, что никому не нравится
...
Для решения данной задачи мы пишем метод FindUniqueVowels, которая принимает string и Hashset со всеми русскими гласными. Мы разделяем текст на слова и записываем данные о гласных. Считаем кол-во гласных в тексте и добавляем их если они встречаются в тексте с переодчиностью в <=1
